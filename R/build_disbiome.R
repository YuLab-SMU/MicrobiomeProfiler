## Disbiome external-data builder.
##
## These helpers used to live in `data-raw/build_disbiome.R`. `data-raw/` is
## listed in `.Rbuildignore`, so nothing defined there is reachable from the
## installed package and the corresponding tests could never run under
## `R CMD check`. Keeping the implementation here (internal, not exported) means
## `tests/testthat/test_build_disbiome.R` exercises shipped code, while
## `data-raw/build_disbiome.R` stays a thin loader for the update workflow.
