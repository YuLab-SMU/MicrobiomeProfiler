...
#'
mod_COGenrichment_ui <- function(id,label = "Input: COG list",universelist = list("cog","customer_defined_universe")){
  ns <- NS(id)
  tagList(
    tags$div(
      textAreaInput(ns("genelist"),label=label,placeholder = "COG0001\nCOG0007\n..."),
...
mod_COGenrichment_server <- function(id){
  moduleServer( id, function(input, output, session){
    ...
    observeEvent(input$ex,{
      updateTextAreaInput(session, "genelist", value = paste0(cog_20$V1[1:50], collapse = "\n"))
    })
        ...
        if(!is.null(kk)){
          ...
          output$dotPlot <- renderPlot({
            ...
            if(input$update == 0){
              ...
              dotplot(kk) + ggplot2::scale_color_gradient(low=input$lowcolor,high=input$highcolor) + guides(color = guide_colorbar(reverse = TRUE))
            } else{
