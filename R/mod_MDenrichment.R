...
mod_MDenrichment_ui <- function(id,label = "Input: Microbe NCBI Taxid list",universelist = list("disbiome","customer_defined_universe")){
  ns <- NS(id)
  tagList(
    tags$div(
      textAreaInput(ns("genelist"),label=label,placeholder = "1591\n853\n39491\n..."),
...
mod_MDenrichment_server <- function(id){
  moduleServer( id, function(input, output, session){
    ...
    observeEvent(input$ex,{
      updateTextAreaInput(session, "genelist", value = paste0(disbiome_data$organism_ncbi_id[1:50], collapse = "\n"))
    })
