...
Currently, `enrichBugSigDB()` downloads BugSigDB artifacts on demand, and
`enrichMDA()` / `gseMDA()` use remote-first Disbiome data with fallback to the
bundled internal dataset when the remote resource is unavailable.
