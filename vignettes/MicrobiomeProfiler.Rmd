...
+ eggNOG-based KEGG pathway enrichment analysis
...
Some annotation resources are now distributed from the package GitHub Pages
site instead of being bundled directly in the package tarball. This keeps the
installed package smaller while allowing data artifacts to be refreshed
independently.

You can inspect configured remote datasets with:
...
Currently, `enrichBugSigDB()` and `enrichEggNOG()` download external artifacts
on demand, and `enrichMDA()` / `gseMDA()` use remote-first Disbiome data with
fallback to the bundled internal dataset when the remote resource is
unavailable.
